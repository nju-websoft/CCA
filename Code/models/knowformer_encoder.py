...
def attention(query, key, value, mask, attention_dropout_layer, trustworthy):
    d_k = query.size(-1)
    
    # scores.shape: (batch_size, head_size, seq_size=3, seq_size=3)
    scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k)
    if mask is not None:
        scores = scores.masked_fill(mask == 0, -1e5)
    # p_attn.shape: (batch_size, head_size, seq_size, seq_size)
    p_attn = nn.functional.softmax(scores, dim=-1)
    trustworhty = None
    if trustworthy != None:
        refinement = (1- trustworthy) * (- 0.1)
        refinement = refinement.unsqueeze(1)
        refinement = refinement.unsqueeze(1)
        
        refinement = refinement.expand(p_attn.shape).float()
        p_attn = nn.functional.softmax(scores + refinement, dim = -1)     
    else:
        p_attn = nn.functional.softmax(scores, dim = -1)
    ...
    attention_drop_mask = (p_attn < 0.02)
    p_attn = p_attn.masked_fill(attention_drop_mask, 0)
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask, neighbor_trustworthy):
        ...
        x, self.attn = attention(query, key, value, mask=mask,
                                 attention_dropout_layer=self.attention_dropout_layer, trustworthy = neighbor_trustworthy)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask, neighbor_trustworthy):
        ...
        x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, mask, neighbor_trustworthy))
...
class Encoder(nn.Module):
    ...
    def forward(self, x, neighbor_trustworthy, each_layer, mask):
        # x.shape: (batch_size, seq_size=3, d_model)
        # mask.shape: (batch_size, seq_size=3, seq_size=3)
        layer_result = []
        for i in range(len(self.layers)):
            layer = self.layers[i]
            x = layer(x, mask, neighbor_trustworthy)
            if each_layer:
                layer_result.append(self.norm(x))
        if each_layer:
            return layer_result
        else:
            return self.norm(x)
